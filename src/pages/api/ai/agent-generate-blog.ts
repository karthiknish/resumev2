...
async function fetchUrlContent(url: string): Promise<string> {
  try {
    ...
    console.error(`[Agent Mode] Error fetching URL:`, (error as Error).message);
    throw error;
  }
}
  ...
  try {
    ...
    if (url?.trim()) {
      try {
        ...
        console.error('[Agent Mode] URL fetch failed, proceeding with context only:', (urlError as Error).message);
      }
    }
