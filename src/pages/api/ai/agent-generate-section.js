// src/pages/api/ai/agent-generate-section.js
...
export default async function handler(req, res) {
  ...
  try {
    const { sectionId, sectionHeading, sectionPoints, blogTitle, styleConfig } = req.body;
    ...
    const prompt = `
      ...
      **HTML Formatting Requirements:**
      - Start with a <h2> tag containing the section heading: <h2>${sectionHeading}</h2>
    ...
    return res.status(500).json({
      success: false,
      message: error.message || "Error generating section content",
    });
  }
}
