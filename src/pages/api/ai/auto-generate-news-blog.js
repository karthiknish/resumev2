...
const parseAIResponse = (responseText) => {
  ...
  if (!output.description) output.description = output.body.substring(0, 160);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Failed to fetch news from GNews: ${newsError.response?.data?.errors?.join(", ") || newsError.message}`);
    }
    ...
    **OUTPUT FORMAT:**
    ...
    if (error.message.includes("parsing")) {
      errorMessage = "AI response generated, but failed to parse the structure.";
