...
export default async function handler(req, res) {
  ...
  try {
    // Check usage limit from Firebase
    ...
    if (currentCount >= dailyLimit) {
      console.warn(`GNews API limit reached for today (${today}). Count: ${currentCount}`);
    ...
    if (response.status !== 200 || !response.data || !response.data.articles) {
      ...
      throw new Error(response.data?.errors?.join(", ") || "Failed to fetch news from GNews");
    }
    ...
    console.error("Error fetching trending news from GNews:", error.response?.data || error.message);
    const errorMessage = error.response?.data?.errors?.join(", ") || error.message || "Error fetching trending news";
