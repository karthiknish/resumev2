...
export default async function handler(req: NextApiRequest, res: NextApiResponse): Promise<void> {
  ...
  try {
    const { topic, outline, tone, length, keywords } = req.body as GenerateSectionsRequest;
    ...
    if (!effectiveOutline || !effectiveOutline.headings || effectiveOutline.headings.length === 0) {
      const outlinePrompt = `
        ...
        Generate a logical and comprehensive blog post outline for
        topic: "${topic}".
    ...
    for (let i = 0; i < effectiveOutline.headings.length; i++) {
      ...
      if (isIntroduction) {
        ...
        sectionPrompt = `
          ...
          Do not include any preamble or explanations outside of HTML.
        `;
      }
