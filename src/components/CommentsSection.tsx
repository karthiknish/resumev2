...
import { AppSession } from "@/lib/apiUtils";
    ...
    try {
      ...
      if (response.ok) {
        ...
        const commentToAdd: Comment = {
          ...data.data,
          authorName: typedSession?.user?.name || anonymousName.trim() || "Anonymous",
          authorImage: typedSession?.user?.image || null,
      ...
      const errorMessage = err instanceof Error ? err.message : "Could not post comment.";
      setError(errorMessage);
              ...
              >
                <div className="relative">
                  <Image
                    ...
                    width={56}
                    height={56}
                    className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl border-4 border-slate-200 shadow-sm object-cover"
                  />
